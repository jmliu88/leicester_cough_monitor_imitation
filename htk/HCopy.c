...
int mainHCopy(int argc, char *argv[])
{
   static int ft=1;
...
{
	trace  = 0;         /* Trace level */
	trStr = &trList;   /* ptr to it */
	traceWidth = 70;     /* print this many chars before wrapping ln */
	nParm = 0;            /* total num params */
	srcFF     = UNDEFF;   /* I/O configuration options */
	tgtFF     = UNDEFF;
	srcLabFF  = UNDEFF;
	tgtLabFF  = UNDEFF;
	srcPK       = ANON;
	...
	saveAsVQ = FALSE;
	swidth0 = 1;
	st=0.0;            /* start of samples to copy */
	en=0.0;            /* end of samples to copy */
	xMargin=0.0;       /* margin to include around extracted labs */
	stenSet=FALSE;   /* set if either st or en set */
	labstidx=0;          /* label start index (if set) */
	labenidx=0;          /* label end index (if set) */
	curstidx=0;          /* label start index (if set) */
	curenidx=0;          /* label end index (if set) */
	labRep=1;            /* repetition of named label */
	auxLab = 0;          /* auxiliary label to use (0==primary) */
	chopF = FALSE;   /* set if we should truncate files/trans */
	labName = NULL;    /* name of label to extract (if set) */
	useMLF=FALSE;    /* set if we are saving to an mlf */
	labF=FALSE;      /* set if we should  process label files too */
	labDir = NULL;     /* label file directory */
	outLabDir = NULL;  /* output label dir */
	off = 0.0;         /* length of files appended so far */
}
/* ----------------------------------------------------------- */
/*                      END:  HCopy.c                          */
/* ----------------------------------------------------------- */
