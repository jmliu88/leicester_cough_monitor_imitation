...
static void zwangModify();
static void Initialise(void);
static void DoRecognition(void);
static void DoAlignment(void);
int mainHVite(int argc, char *argv[])
{
	static int ft=1;
   char *s;
   zwangModify();
...
{
	trace = 0;
	nToks = 0;             /* Number of tokens for N best */
	nTrans = 1;            /* Number of transcriptions for N best */
	states = FALSE;    /* Keep track of state alignment */
	models = FALSE;    /* Keep track of model alignment */
	wdNetFn = NULL;      /* Word level lattice */
	hmmDir = NULL;      /* directory to look for hmm def files */
	hmmExt = NULL;      /* hmm def file extension */
	loadLabels = FALSE; /* Load network for each file */
	loadNetworks = FALSE; /* Load network for each file */
	bndId = NULL;        /* Boundary word for alignment */
	labDir = NULL;      /* output label file directory */
	labExt = "rec";     /* output label file extension */
	labForm = NULL;     /* output label reformat */
	latForm = NULL;     /* output lattice format */
	labInDir = NULL;    /* input network/label file directory */
	latExt = NULL;      /* output lattice file extension */
	labFileMask = NULL; /* mask for reading lablels (lattices) */
	dfmt=UNDEFF;    /* Data input file format */
	ifmt=UNDEFF;    /* Label input file format */
	 ofmt=UNDEFF;    /* Label output file format */
	saveAudioOut=FALSE;/* Save rec output from direct audio */
	roPrefix=NULL;      /* Prefix for direct audio output name */
	roSuffix=NULL;      /* Suffix for direct audio output name */
	roCounter = 0;         /* Counter for audio output name */
	replay = FALSE;    /* enable audio replay */
	lmScale = 1.0;      /* bigram and log(1/NSucc) scale factor */
	wordPen = 0.0;   /* inter model propagation log prob */
	prScale = 1.0;      /* pronunciation scale factor */
	/* Pruning */
	genBeam = -LZERO;/* genBeam threshold */
	genBeamInc  = 0.0;       /* increment         */
	genBeamLim = -LZERO;     /* max value       */
	nBeam = 0.0;     /* nBeam threshold */
	wordBeam = -LZERO;/* word-end pruning threshold */
	tmBeam = 10.0;    /* tied mix prune threshold */
	maxActive = 0;         /* max active phone instances */
	maxM = 0;              /* max mixtures in any model */
	update = 0;            /* Perfom MLLR & update every n utts */
	saveBinary=FALSE;  /* Save tmf in binary format */
	nParm = 0;            /* total num params */
}
/* ----------------------------------------------------------- */
/*                      END:  HVite.c                          */
/* ----------------------------------------------------------- */
