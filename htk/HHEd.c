...
static void zwangEdit();
static void Initialise(char *hmmListFn);
int mainHHEd(int argc, char *argv[])
{
	static int ft=1;
   char *s, *editFn;
...
static void zwangEdit()
{
	...
	newExt = NULL;     /* extension of new edited hmm files */
	noAlias = FALSE;  /* set to zap all aliases in hmmlist */
	inBinary = FALSE; /* set to save models in binary */
	mmfFn  = NULL;     /* output MMF file, if any */
	cmdTrace    = 0;     /* trace level from command line */
	trace    = 0;        /* current trace level */
	joinSize=0;                 /* number of mixes in a joined pdf */
	badGC = FALSE;          /* set TRUE if gConst out of date */
	occStatsLoaded = FALSE; /* set when RO/LS has loaded occ stats */
	outlierThresh = -1.0;     /* outlier threshold set by RO cmd */
	lastCommand=0;              /* index of previous command */
	equivState = TRUE;      /* TRUE if states can be equivalent */
	useModelName = TRUE;    /* Use base-phone name as tree name */
	saveHMMSet   = TRUE;    /* Save the HMMSet */
	nParm = 0;            /* total num params */
	treeMerge = TRUE; /* After tree spltting merge leaves */
	useLeafStats = TRUE; /* Use leaf stats to init macros */
	applyVFloor = TRUE; /* apply modfied varFloors to vars in model set */ 
	qHead = NULL;      /* Head of question list */
	qTail = NULL;      /* Tail of question list */
	treeList = NULL;   /* list of trees */
}
