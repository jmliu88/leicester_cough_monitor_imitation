...
static void Initialise1(void);
static void Initialise2(void);
static void LoadFile(char *fn);
int mainHRest(int argc, char *argv[],char *inScorePath)
{
	static int ft=1;
	int i;
   char *datafn, *s;
  ...
  CreateHMMSet(&hset,&gstack,FALSE);
   ...
   if (nSeg < minSeg)
   {
      HRError(2221,"HRest: Too Few Training Examples [%d]",nSeg);
	Dispose(hset.hmem,hset.mtab);
	zwangInitParmClear();
	zwangInitShellClear();
	ResetHeap(&gstack);
	ResetHeap(&segmentStack);
	ResetHeap(&alphaBetaStack);
	ResetHeap(&accsStack);
...
static void PrintInitialInfo(void)
{   
...
static void InitSegStore(BufferInfo *info)
{
   //Observation obs;
   Boolean eSep;
...
void ReEstimateModel(void)
{
   ...
   do {        /*main re-est loop*/   
      ...
      for (seg=1;seg<=nSeg;seg++) {
		 ...
		 {
			segProb=LZERO;//ap=LZERO;
            if (trace&T_TOP) 
               printf("Example %d skipped\n",seg);
			}
		if(scores)
		{
			fprintf(scores,"%f\n",segProb);//fprintf(scores,"%lf\n",ap);
		}
      }
	  if(scores)
		  fclose(scores);
      if (nTokUsed==0)
	  {
         HRError(2226,"ReEstimateModel: No Usable Training Examples");
		//--zw
		 return;
		 //--zw
	  }
...
static void zwangModify()
{
	segLab = NULL;     /* segment label if any */
	segId  = NULL;     /* and its index */
	labDir = NULL;     /* label file directory */
	labExt = "lab";    /* label file extension */
	outDir = NULL;     /* output macro file directory, if any */
	maxIter  = 20;       /* max iterations in parameter estimation */
	epsilon = 1.0E-4;   /* convergence criterion */
	minSeg    = 3;        /* min segments to train a model */
	firstTime = TRUE; /* Flag used to enable InitSegStore */
	saveBinary = FALSE;  /* save output in binary  */
	dff=UNDEFF;    /* data file format */
	lff=UNDEFF;    /* label file format */
	minVar  = 0.0;      /* minimum variance */
	mixWeightFloor=0.0; /* Floor for mixture weights */
	tMPruneThresh = 10.0;    /* tied mix prune threshold */
	outfn=NULL;         /* output definition file name */ 
	trace    = 0;        /* Trace level */
	nParm = 0;               /* total num params */
	segReject = FALSE;//TRUE; /* Enable short train segment rejection */
	vDefunct=0.0;       /* variance below which mixture defunct */
	segIdx=1;  /* Between call handle on latest seg in segStore */  
	prevSegIdx=1;
}
/* ----------------------------------------------------------- */
/*                      END:  HRest.c                          */
