...
static void EstimateModel(void);
static void SaveModel(char *outfn);
...
static void zwangModify()
{
	...
	segId  = NULL;        /* and its id */
	labDir = NULL;        /* label file directory */
	maxIter  = 20;          /* max iterations in param estimation */
	epsilon = 1.0E-4;      /* convergence criterion */
	minVar  = 1.0E-2;      /* minimum variance */
	mixWeightFloor=0.0;    /*Floor for mixture/discrete prob weights*/
	minSeg    = 1;           /* min segments to train a model */
	newModel = TRUE;    /* enable initial uniform segmentation */
	...
	outfn=NULL;            /* output HMM file name (name only) */
	outDir=NULL;           /* HMM output directory */
	trace = 0;               /* Trace level */
