...
static void Initialise(FBInfo *fbInfo, MemHeap *x, HMMSet *hset, char *hmmListFn);
int mainHERest(int argc, char *argv[])
{
	static int ft=1;
   char *datafn=NULL;
   char *datafn2=NULL;
   ...
   ResetHeap(&fbInfo->ab->abMem);
	...
	DeleteHeap(&fbInfo->ab->abMem);
	DeleteHeap(&utt->dataStack);
	DeleteHeap(&utt->transStack);
   ResetHeap(&uttStack);
...
static void PrintStats(HMMSet *hset,FILE *f, int n, HLink hmm, int numEgs)
{
   WtAcc *wa;
   char buf[MAXSTRLEN];
   StateInfo *si;
...
static void FloorMixes(HMMSet *hset, MixtureElem *mixes, int M, float floor)
{
...
static void zwangModify()
{
	labDir = NULL;     /* label (transcription) file directory */
	hmmDir = NULL;     /* directory to look for hmm def files */
	hmmExt = NULL;     /* hmm def file extension */
	newDir = NULL;     /* directory to store new hmm def files */
	newExt = NULL;     /* extension of new reestimated hmm files */
	minVar  = 0.0;      /* minimum variance (diagonal only) */
	mixWeightFloor=0.0; /* Floor for mixture weights */
	minEgs    = 3;        /* min examples to train a model */
	parMode   = -1;       /* enable one of the // modes */
	stats = FALSE;    /* enable statistics reports */
	mmfFn  = NULL;     /* output MMF file, if any */
	trace     = 0;        /* Trace level */
	saveBinary = FALSE;  /* save output in binary  */
	ldBinary = TRUE;        /* load/dump in binary */
	dff=UNDEFF;       /* data file format */
	lff=UNDEFF;       /* label file format */
	updateMode = UPMODE_UPDATE; /* dump summed accs, update models or do both? */
	nParm = 0;               /* total num params */
	al_hmmUsed = FALSE;   /* Set for 2-model ReEstimation */
	/* Global Data Structures - valid for all training utterances */
	pruneInit = NOPRUNE;    /* pruning threshold initially */
	pruneInc = 0.0;         /* pruning threshold increment */
	pruneLim = NOPRUNE;     /* pruning threshold limit */
	minFrwdP = NOPRUNE;         /* mix prune threshold */
	firstTime = TRUE;    /* Flag used to enable creation of ot */
	twoDataFiles = FALSE; /* Enables creation of ot2 for FB
											training using two data files */
	totalT=0;       /* total number of frames in training data */
	totalPr=0;   /* total log prob upto current utterance */
	maxSpUtt = 0;
	varFloorPercent = 0;
	labFileMask = NULL;
